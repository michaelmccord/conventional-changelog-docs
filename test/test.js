...
betterThanBefore.setups([
  function () {
    ...
    gitDummyCommit(['build: first build setup', 'MAJOR CHANGE: New build system.'])
    gitDummyCommit(['ci(travis): add TravisCI pipeline', 'MAJOR CHANGE: Continuously integrated.'])
    gitDummyCommit(['feat: amazing new module', 'MAJOR CHANGE: Not backward compatible.'])
    gitDummyCommit(['fix(compile): avoid a bug', 'MAJOR CHANGE: The Change is huge.'])
  ...
  function () {
    gitDummyCommit(['build(npm): edit build script', 'MAJOR CHANGE: The Change is huge.'])
    gitDummyCommit(['ci(travis): setup travis', 'MAJOR CHANGE: The Change is huge.'])
    gitDummyCommit(['docs(readme): make it clear', 'MAJOR CHANGE: The Change is huge.'])
    gitDummyCommit(['style(whitespace): make it easier to read', 'MAJOR CHANGE: The Change is huge.'])
    gitDummyCommit(['refactor(code): change a lot of code', 'MAJOR CHANGE: The Change is huge.'])
    gitDummyCommit(['test(*): more tests', 'MAJOR CHANGE: The Change is huge.'])
  },
  function () {
  ...
  function () {
    ...
    gitDummyCommit(['build(deps): bump @dummy/package from 7.1.2 to 8.0.0', 'MAJOR CHANGE: The Change is huge.'])
  },
  function () {
...
describe('angular preset', function () {
  it('should work if there is no semver tag', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk) {
        ...
        expect(chunk).to.include('MAJOR CHANGE')
  ...
  it('should not discard commit if there is MAJOR CHANGE', function (done) {
    preparing(5)
  ...
  it('should work if there is a semver tag', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk, enc, cb) {
        ...
        expect(chunk).to.not.include('MAJOR')
